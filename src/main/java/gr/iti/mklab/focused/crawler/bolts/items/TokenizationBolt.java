...
public class TokenizationBolt extends BaseRichBolt {
	...
	private List<String> tokenize(String text) throws IOException {
        ...
        List<String> tokens = new ArrayList<String>();
        TokenStream stream = new StandardTokenizer(new StringReader(text)); 
        StopFilter stopFilter = new StopFilter(stream, StandardAnalyzer.STOP_WORDS_SET);
        ...
        if(_maxNgrams > 1) {
        	...
        	stream = new LowerCaseFilter(sf);
        }
        else {
        	stream = new LowerCaseFilter(stopFilter);
        }
